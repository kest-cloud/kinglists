                ...
                child: Padding(
                  ...
                  child: CupertinoTextField(
                    ...
                    onChanged: (val) {
                      setState(() {
                        var keys =
                            allIconsMap.keys.where((key) => key.contains(val));
                        iconsMap = Map.fromEntries(keys.map(
                            (key) => MapEntry(key, FontAwesomeIconsMap[key])));
                      });
                    },
                  ),
                ),
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: <Widget>[
              Transform.scale(
                  scale: 2, child: Icon(FontAwesomeIconsMap[iconString])),
              SizedBox(height: 24)
            ],
          ),
          actions: <Widget>[
...
class IconGridView extends StatelessWidget {
  ...
  IconGridView({this.scrollController, this.iconsMap, this.onIconTapped})
      : assert(iconsMap != null);
