//import 'package:KingsList/resources/db_provider.dart';
import 'package:rxdart/rxdart.dart';

import 'package:kingslist/resources/repository.dart';
...
class ProjectBloc {
  ...
  void fetchAllProjects() async {
    ...
    _todoFetcher.add(_currentProject.tasks
        .where((t) => t.status == TaskStatus.todo)
        .toList());
    _doingFetcher.add(_currentProject.tasks
        .where((t) => t.status == TaskStatus.doing)
        .toList());
    _doneFetcher.add(_currentProject.tasks
  ...
  void getProjectById(String uid) {
    _currentProject =
        _allProjects.singleWhere((p) => p.uid == uid, orElse: null);
  ...
  void updateTaskStatus(Task task) {
    ...
    var t = _currentProject.tasks
        .singleWhere((t) => t.uid == task.uid, orElse: () => null);
    switch (task.status) {
